...
function timeSpanToString(startDate, endDate) {
  ...
  function formatDate(date) {
    const HH = leadZero(date.getUTCHours(), 2);
    const mm = leadZero(date.getUTCMinutes(), 2);
    const ss = leadZero(date.getUTCSeconds(), 2);
    const sss = leadZero(date.getUTCMilliseconds(), 3);

    return `${HH}:${mm}:${ss}.${sss}`;
  }

  const dif = new Date(endDate).getTime() - new Date(startDate).getTime();
  return formatDate(new Date(dif));
}
